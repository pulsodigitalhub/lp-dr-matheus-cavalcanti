...
(function () {
  ...
  var backdrop, form, nameInput, phoneInput, errorEl, submitBtn, submitLabel;
  var lastFocus = null;
  ...
  function buildModal() {
    ...
    backdrop.innerHTML =
      '<div class="lead-modal" role="dialog" aria-modal="true" aria-labelledby="lead-modal-title">' +
        ...
        '<form class="lead-modal-form" novalidate>' +
          ...
          '<p class="lead-modal-privacy">Usamos seu nome e telefone apenas para o agendamento. Veja a <a href="/politicas-de-privacidade/">Política de Privacidade</a>.</p>' +
          '<p class="lead-modal-skip"><button type="button" class="lead-modal-skip-btn">Prefiro ir direto para o WhatsApp</button></p>' +
    ...
    backdrop.querySelector('.lead-modal-skip-btn').addEventListener('click', irParaWhatsApp);
  ...
  function onSubmit(event) {
    ...
    if (!nome || nome.length < 2) { showError('Informe seu nome para continuar.'); nameInput.focus(); return; }
    if (!isValidBrazilPhone(telefoneDigits)) { showError('Informe um telefone válido com DDD.'); phoneInput.focus(); return; }

    var message = 'Olá, gostaria de agendar uma avaliação com ' + current.doctor + '.\n\nNome: ' + nome + '\nTelefone: ' + formatPhone(telefoneDigits);
    ...
    window.dataLayer.push({ event: 'lead_form_submit', form_name: 'lead_modal_matheus_cavalcanti' });
    ...
    fetch(WEBHOOK_URL, {
      ...
      keepalive: true
    }).catch(function () { /* O webhook é melhor-esforço e não bloqueia o WhatsApp. */ });
